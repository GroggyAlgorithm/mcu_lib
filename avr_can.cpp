...
uint8_t CANRaw::setNumTXBoxes(uint8_t txboxes) {
	...
	//Initialize remaining Mob as RX boxes
	for (c = 0; c < CANMB_QUANTITY - numTXBoxes; c++) {
		// mailbox_set_id(c, 0x0, false);
		// mailbox_set_accept_mask(c, 0x7FF, false);
		
		setRXFilter(c, 0, 0, false); //TR - Added/changed. It was not working to have both rx and tx mailboxes without this
	}
...
/**
* \brief Check whether there are received can bus frames in the buffer
*
* \retval true if there are frames waiting in buffer, otherwise false
*/
bool CANRaw::rx_avail() {
	bool isWaiting =  (rx_buffer_head != rx_buffer_tail)?true:false;
	return isWaiting;
}
...
#endif   
{
    uint8_t savedMOB;                                           // Save and restore the index value, in case interrupt occurred during an ongoing operation..
	...
	ISR(CAN_TOVF_vect)
#endif
{                                                               // Should never get here, as Overflow interrupts are not enabled - -
