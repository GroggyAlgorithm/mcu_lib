...
/*
  ...
  Modified by Al Thomason to support CAN enabled AVR CPUs:
...
#define CAN_MAILBOX_RX_OVER           0x02  //! Message overwriting happens or there are messages lost in different receive modes.
...
typedef struct {
    uint32_t  baudrate;
	uint8_t   canbt1;                                 // Just hold the pre-calculated register values.  See ATmegaxxM1 datasheet.
	uint8_t   canbt2; 
	uint8_t   canbt3;  
} can_bit_timing_t;
...
/** Values of bit time register for different baud rates and different CPU frequencies.  */
